...
def evaluate_polynom(polynom_str, x):
    polynom_str = re.sub(r'^[LP]\(x\) *= *', '', polynom_str)
    
    polynom_str = re.sub(r'\)(?=\()', r')*', polynom_str)
    polynom_str = re.sub(r'\)(?=[0-9])', r')*', polynom_str)
    polynom_str = re.sub(r'(?<=[0-9])\(', r'*(', polynom_str)
    
    polynom_str = polynom_str.replace('x', f'({x})')
    
    polynom_str = re.sub(r'\s+', '', polynom_str)
    
    print("Evaluated string:", polynom_str)
    
    return eval(polynom_str)